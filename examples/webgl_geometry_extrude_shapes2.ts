...
function addGeoObject(group, svgObject) {
  const thePaths = svgObject.paths;
  const theAmounts = svgObject.amounts;
  const theColors = svgObject.colors;
  const theCenter = svgObject.center;
  for (let i = 0; i < thePaths.length; ++i) {
    const path = $d3g.transformSVGPath(thePaths[i]);
    const color = new THREE.Color(theColors[i]);
    const material = new THREE.MeshLambertMaterial({
      color: color,
      emissive: color,
    });
    const amount = theAmounts[i];
    const simpleShapes = path.toShapes(true);
    for (let j = 0; j < simpleShapes.length; ++j) {
      const simpleShape = simpleShapes[j];
      const shape3d = simpleShape.extrude({
        amount: amount,
        bevelEnabled: false
      });
...
function init3d(){
  ...
  /// backgroup grids
  const helper = new THREE.GridHelper(80, 10);
...
/// Main
let renderer: THREE.WebGLRenderer, stats: Stats;
